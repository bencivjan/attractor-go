...
Communication nodes (`submit_for_evaluation`, `evaluation_feedback`) mark the boundary with the evaluator pipeline. In standalone mode they are pass-throughs. When run via `FactoryRunner`, the submit node's output becomes the evaluator's input, and evaluator rejection feedback is injected into `$evaluator_feedback` on the next developer iteration.
...
Writes production-ready code following the sprint definitions. If `$evaluator_feedback` is present in context (injected by `FactoryRunner` after an evaluator rejection), the implementer addresses every issue the evaluator raised before proceeding with sprint work.
...
With evaluator via FactoryRunner:
```go
factory := &factory.FactoryRunner{
    Registry:      registry,
    MaxRejections: 3,
}
outcome, err := factory.RunWithGoal(ctx, "build a feature", "/path/to/logs")
```

## Source

[`attractor/pipelines/developer.dot`](developer.dot)
